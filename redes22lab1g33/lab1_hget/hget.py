...
def connect_to_server(server_name):
    ...
    ip_address = socket.gethostbyname(server_name)  # Se utiliza la función gethostbyname de python para obtener la dirección IP
                                                    # del servidor
    ...
    s =socket.socket(socket.AF_INET, socket.SOCK_STREAM) # Se utiliza AF_INET que es el dominio del conector, en éste caso IPv4. Y el
                                                         # STREAM es el tipo de conector, en éste caso el protocolo TCP.
                                                         
    s.connect((ip_address, 80))                         # Luego iniciamos la conección con un host en un puerto, en éste caso 80
